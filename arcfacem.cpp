...
float arcfacem::Compare(float* prob1, float* prob2)
{
    ...
    {
    p1[i]=prob1[i];
    std::cout<<"first="<<p1[i]<<std::endl;
    }
    std::cout<<prob1<<std::endl;
    cv::Mat out1(128, 1, CV_32FC1, p1);
    cv::Mat out_norm1;
    cv::normalize(out1, out_norm1);
    std::cout<<"calculate first over"<<std::endl;

    float p2[128];
    for(int i=0;i<128;i++)
    {
    p2[i]=prob2[i];
    std::cout<<"second="<<p2[i]<<std::endl;
    }
    cv::Mat out2(1,128, CV_32FC1, p2);
    cv::Mat out_norm2;
    cv::normalize(out2, out_norm2);
    std::cout<<"calculate second over"<<std::endl;
    cv::Mat res = out_norm2 * out_norm1;
    std::cout<<"calculate over:"<<*(float*)res.data<<std::endl;
